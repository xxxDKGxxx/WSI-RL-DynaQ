...
def plot_policy_dynamic_target(
    ...
    start_t_dir = getattr(env, "target_start_dir", 1)

    ax.text(sc, sr, "S", ha="center", va="center", fontsize=14, fontweight="bold")
    ax.text(tc, tr, "T", ha="center", va="center", fontsize=14, fontweight="bold")

    for s in range(env.num_states):
        ar, ac, current_tr, current_tc, current_t_dir = env.decode_state(s)

        if (current_tr, current_tc) != (tr, tc) or current_t_dir != start_t_dir:
            continue
...
def plot_value_heatmap_dynamic_target(
    ...
    for s in range(env.num_states):
        ar, ac, current_tr, current_tc, current_t_dir = env.decode_state(s)
        if (current_tr, current_tc) == (tr, tc) and current_t_dir == start_t_dir:
            V_grid[ar, ac] = V[s]
...
def render_episode_frames(
    ...
    for t, (s, a_intended, a_exec, r, s_next, done) in enumerate(trajectory):
        ...
        if "MultipleTarget" in env_name:
            ...
            gr, gc = env.state_to_row_column(env.goal_state)
            ax.text(
                gc, gr, "G", ha="center", va="center", fontsize=14, fontweight="bold"
            )
            for cliff_s in env.cliff_states:
                cr, cc = env.state_to_row_column(cliff_s)
                ax.text(
            ...
            r_next, c_next, tr, tc, _ = env.decode_state(s_next)
            ax.text(
                tc,
                tr,
                "T",
        ...
        else:  # Standard SlipperyGridWorld
            gr, gc = env.goal_row_column
            ax.text(
                gc, gr, "G", ha="center", va="center", fontsize=14, fontweight="bold"
            )
        ...
        if show_executed_action:
            ax.set_title(
                f"t={t}  intended={ARROWS[a_intended]}  executed={ARROWS[a_exec]}  r={
                    r:.2f}  done={done}"
            )
...
def run_to_gif(
    ...
    frames = render_episode_frames(
        env, roll["trajectory"], out_dir="frames", prefix="ep"
    )
